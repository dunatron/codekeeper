    ...
    return (
      <form className={classes.createNewsForm} noValidate autoComplete="off">
        ...
        <Button
          ...
          raised
          color="primary"
          onClick={() => this._createCodeSample()}>
          Send
  ...
  _createCodeSample = async () => {
    ...
    await this.props.createCodeSampleMutation({
      ...
      update: (store, { data: { createCode_Sample } }) => {
        ...
        console.log(data)
        console.log("Create Code example ", createCode_Sample)
        // data.readCodeSamples.edges.splice(0, 0, createCode_Sample)
        data.readCodeSamples.edges.splice(0, 0, {
          node: {
            ...createCode_Sample,
            VotesOnCode: [],
            __typename: "Code_Sample",
          },
          __typename: "readCodeSamplesEdge",
        })
        store.writeQuery({
...
const CREATE_CODE_SAMPLE_MUTATION = gql`
  # 2
  mutation CreateCodeSampleMutation(
    $title: String
    $codeString: String
    $createdById: ID
  ) {
    createCode_Sample(
      Input: { Title: $title, CodeBody: $codeString, OwnerID: $createdById }
    ) {
      ID
      Created
      Title
